...
#define SPI_SLAVE_DMA_CLK            RCC_AHBPERIPH_DMA2
#define SPI_SLAVE_Rx_DMA_Channel     DMA2_Channel1
#define SPI_SLAVE_Rx_DMA_FLAG        DMA2_FLAG_TC1
#define SPI_SLAVE_Tx_DMA_Channel     DMA2_Channel5
#define SPI_SLAVE_Tx_DMA_FLAG        DMA2_FLAG_TC5  
#define SPI_SLAVE_DR_Base            
...
int main(void)
{
  ...
  SPI_InitStructure.SPI_CPOL = SPI_CPOL_LOW;      // 时钟空闲，低电平
  SPI_InitStructure.SPI_CPHA = SPI_CPHA_2EDGE;    // 数据读取在时钟上升沿
  SPI_InitStructure.SPI_NSSSEL = SPI_NSSSEL_SOFT; // 软件控制片选脚
  SPI_InitStructure.SPI_MCLKP = SPI_MCLKP_32;      // 分频系统时钟，120m / 16 < 6.5m, 1101最小的选项支持最大的频率6.5m
  SPI_InitStructure.SPI_FirstBit = SPI_FIRSTBIT_MSB;  // 高位在前
  SPI_InitStructure.SPI_CPOLY = 7;        // 无校验的话可以忽略
  ...
  SPI_I2S_DMAEnable(SPI_MASTER, SPI_I2S_DMA_RX, ENABLE); // 主机DMA接收
  ...
  memset(SPI_SLAVE_Buffer_Rx, NULL, sizeof(SPI_SLAVE_Buffer_Rx));
  while (1)
  {}
}
