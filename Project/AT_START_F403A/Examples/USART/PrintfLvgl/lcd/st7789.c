/*
 * @Author: ZhiQiang
 * @Date: 2023-02-06 11:48:36
 * @CodedFormat: utf8
 * @LastEditors: ZhiQiang
 * @LastEditTime: 2023-02-06 13:51:19
 * @FilePath: \MDK_v5e:\Desktop\arterytek\AT32F4xx_StdPeriph_Lib_V1.2.9\Project\AT_START_F403A\Examples\USART\PrintfLcdSpiTest\lcd\st7789.c
 * @Description: 
 */
#include "st7789.h"
#include "at32f4xx.h"
...
void st7789_delay_us(uint16_t us)
{
    while(us)
    {
        uint16_t cnt = 240;
...
static void ST7789_PortPinInit()
{
    GPIO_InitType GPIO_InitStructure;
	RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_GPIOA, ENABLE);
    /* Configure DSPI2 pins: SCK, MISO and MOSI ---------------------------------*/
    GPIO_InitStructure.GPIO_Pins = ST7789_RST_Pin | ST7789_DC_Pin | ST7789_CS_Pin;
    /* Configure SCK and MOSI pins as Alternate Function Push Pull */
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT_PP;
    GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;
	GPIO_WriteBit(ST7789_RST_GPIO_Port, GPIO_InitStructure.GPIO_Pins, Bit_RESET);
    GPIO_Init(ST7789_RST_GPIO_Port, &GPIO_InitStructure);
}

static void ST7789_SpiWriteBytePort(uint8_t Byte)
{
    /* Send SPIz data */
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	size_t cnt = 0;
	while(buff_size) {
		ST7789_SpiWriteBytePort(buff[cnt]);
		cnt++;
		buff_size--;
	}
