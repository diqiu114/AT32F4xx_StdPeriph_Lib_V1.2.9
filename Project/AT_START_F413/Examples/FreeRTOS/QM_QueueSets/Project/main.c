 ...
 * without the task polling each queue in turn to determine which,
 * if any, contains data.
 * 
 * A design that uses a queue set to receive data from multiple 
 * sources is less neat, and less efficient, than a design that
 * achieves the same functionality using a single queue that receives
 * structures. For that reason, it is recommended that queue sets 
 * are only used if design constraints make their use absolutely
 * necessary.
 *
 * Use a queue set:
 * (1)Creating a queue set
 * (2)Adding queues to the set
 * (3)Reading from the queue set to determine which queues within
 *    the set contain data
 *
 * Note:If a queue is a memeber of a queue set then do not read data
 * from the queue unless the queue's handle has first been read from
 * the queue set.
 *
 * Queus and binary semaphores can only be added to a set when they
 * are empty. Counting semaphores can only be added to a set when
 * their count is zero. Queues and semaphores can only be a member
 * of one set at a time.
 *
 * Only read one item from a queue or semaphore each time the queue
 * handle or semaphore handle is returned from a call to xQueueSelectFromSet()
 */
