 ...
 * (1)Event groups allow a task to wait in the Blocked state for a 
 *    combination of one of more events to occur.
 * (2)Event groups unblock all the tasks that were waiting for the
 *    same event, or combination of events, when the event occurs.
 * These unique properies of event groups make them useful for 
...
/* To use xEventGroupSetBitsFromISR(), must define below:
 * (1)INCLUDE_xEventGroupSetBitsFromISR
 * (2)INCLUDE_xTimerPendFunctionCall
 * (3)configUSE_TIMERS
 */
static void TMR_CallBack1(void)
{
