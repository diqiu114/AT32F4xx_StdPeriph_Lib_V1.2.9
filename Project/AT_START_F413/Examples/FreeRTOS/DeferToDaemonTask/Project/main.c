/*
 * Deferring interrupt processing to the daemon task is called 
 * 'centralized deferred interrupt processing'. The xTimerPendFunctionCall()
 * and xTimerPendFunctionCallFromISR() API functions use the same timer
 * command queue to send an 'execute function' command to the daemon task.
 * The function sent to the daemon task is then executd in the context of the 
 * daemon task.
 *
 */
