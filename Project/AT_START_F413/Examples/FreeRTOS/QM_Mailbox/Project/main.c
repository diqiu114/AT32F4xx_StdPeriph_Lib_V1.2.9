/*
 * There is no consensus on terminology within the embedded community,
 * and 'mailbox' will mean different things in different RTOSes. In
 * FreeRTOS the term mailbox is used to refer to a queue that has a
 * length of one. But it has a functional difference to a queue:
 * (1)A queue is used to send data from one task to another task, or
 *    from an interrupt service routine to a task. The sender places
 *    an item in the queue, and the receiver removes the item form 
 *    the queue. The data passes through the queue from the sender
 *    to the receiver.
 * (2)A mailbox is used to hold data that can be read by any task, 
 *    or any interrupt service routine. The data does not pass through
 *    the mailbox, but instead remains in the mailbox until it is overwriteen.
 *    The sender overwrites the value in the mailbox. The receiver reads
 *    the value from the mailbox, but does not remove the value from the mailbox
 */
#include "includes.h"
