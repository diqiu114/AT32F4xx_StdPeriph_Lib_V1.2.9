/*
 * Run-time statistics are intended to be used as a profiling and debugging aid
 * during the development phase of a project. The information they provide is only
 * valid until the counter used as the run-time statistics clock overflows.
 * Collecting run-time statistics will increase the task context switch time
 *
 * Run-time statistics need to measure fractions of a tick period. Therefore, the RTOS
 * tick count is not used as the run-time statistics clock, and the clock is instead 
 * provided by the application code. It is recommended to make the frequency of the 
 * run-time statistics clock between 10 and 100 times faster than the frequency of the 
 * tick interrupt. The faster the run-time statistics clock, the more accurate the 
 * statistics will be, but also the sooner the time value will overflow.
 *
 * Task state:
 *              'E'  -> Executing now
 *              'R'  -> Ready
