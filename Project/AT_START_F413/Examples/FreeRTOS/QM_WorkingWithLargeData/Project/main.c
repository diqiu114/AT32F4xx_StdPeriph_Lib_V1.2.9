/* If the size of the data being stored in the queue is large, 
 * then it is preferable to use the queue to transfer pointers 
 * to the data, rather than copy the data itself into and out of
 * the queue byte by byte. Transferring pointers is more efficient
 * in both processing time and amount of RAM required to create the
 * queue. However, when queuing pointers, extreme care must be taken
 * to ensure that:
 ...
 * sends the address of the buffer to the queue 
 */
static void vTaskSender(void *pvParameters)
{
    char *pcStringToSend;
...
/* A task that receives the address of a buffer from the queue */
static void vTaskReceiver(void *pvParameters)
{
